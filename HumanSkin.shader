...
#define PIE 3.1415926535	
		...
		float4 frag(v2f i) :COLOR
		{
			...
			*/
			...
			float3 ref = texCUBElod(_Cubemap, float4(refDir, 0)).rgb;
			...
			*/
			...
			//float4 c2 = float4(lerp(c, ref, _ReflAmount) *(diff*_Maintint), 0);
			float4 c2 = float4(lerp(c, ref, _ReflAmount) *(diff*_Maintint+all*_SC), 0);
			//float4 c2 = float4(lerp(c, ref, _ReflAmount) *(diff*_Maintint + (all*_SC) / 2)*_nMips + rim + frontrim + spec3 + sss + all*_SC, 1);
